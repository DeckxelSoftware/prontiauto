...
CREATE OR REPLACE FUNCTION obtener_trabajadores(search_par CHAR VARYING, skip_par INTEGER, take_par INTEGER,
                                                sort_by_par CHAR VARYING, sort_direction_par CHAR VARYING,
                                                parametros_par CHAR VARYING, valores_par CHAR VARYING) RETURNS TEXT AS
$$
DECLARE
trabajadores jsonb;
    query        TEXT;
...
INSERT INTO prestamo("sisCreado", "sisActualizado", "sisHabilitado",
...
CREATE OR REPLACE FUNCTION crear_abonos_prestamo(num_cuotas INTEGER, modalidad_descuento CHAR VARYING, monto NUMERIC,
                                                 tasa_interes NUMERIC, id_prestamo INTEGER) RETURNS TEXT AS
$$
DECLARE
...
count := 1;
    ...
    WHILE (count <= num_cuotas)
        LOOP
            last_date_of_month :=
                    (select (date_trunc('month', now()) + concat(count, ' month - 1 day')::interval)::date);
...
INSERT INTO abono_prestamo ("sisCreado", "sisActualizado", "sisHabilitado",
...
BEGIN
count := 1;
    valores_fondo := (SELECT array_agg(co."valorFondo")
                      FROM contrato co
                               INNER JOIN cliente_en_grupo cg ON cg.id = co."idClienteEnGrupo"
                      WHERE cg."idGrupo" = id_grupo);
...
RETURN '{"message":"Se realizo correctamente la actualizacion del monto meta", correcto: true, "sumaMontoMeta": ' ||
       suma_monto_meta || '}';
ROLLBACK;
END;
$$ LANGUAGE 'plpgsql';
...
UPDATE grupo
SET "sisActualizado" = now(), "sumatoriaMontoMeta" = smm
WHERE "nombreGrupo" = ids_grupo[count];
ELSE
                    smm := grupos[count]."sumatoriaMontoMeta" - montos[count];
UPDATE grupo
SET "sisActualizado" = now(), "sumatoriaMontoMeta" = smm
...
create or replace function cuotas_refinanciamiento(cuota_par public.cuota, plan_par public.plan,
                                                   valor_cuota_par NUMERIC, abono_capital_par NUMERIC,
                                                   tasa_administrativa_par NUMERIC, impuesto_par NUMERIC,
                                                   resto_inicio_par NUMERIC, numero_cuotas_inicio_par NUMERIC,
                                                   resto_final_par NUMERIC, pms_par INTEGER,
                                                   id_historico_par INTEGER) returns void as
$$
Declare
...
insert into cuota ("sisActualizado", "sisCreado", "sisHabilitado",
...
CREATE OR REPLACE FUNCTION obtener_licitaciones(search_par CHAR VARYING, skip_par INTEGER, take_par INTEGER,
                                                sort_by_par CHAR VARYING, sort_direction_par CHAR VARYING,
                                                parametros_par CHAR VARYING, valores_par CHAR VARYING) RETURNS TEXT AS
$$
DECLARE
...
INSERT INTO plan ("sisActualizado", "sisCreado", "cuotaMes12", "cuotaMes24", "cuotaMes36", "cuotaMes48",
                  "cuotaMes60", "cuotaMes72", "cuotaMes84", precio, modelo, inscripcion,
                  "sisHabilitado")
VALUES (now(), now(), cuota_mes12, cuota_mes24, cuota_mes36, cuota_mes48, cuota_mes60, cuota_mes72,
        cuota_mes84, precio, modelo, inscripcion, sis_habilitado);
count := count + 1;
END LOOP;
END IF;
...
CREATE OR REPLACE FUNCTION obtener_contratos(search_par CHAR VARYING, skip_par INTEGER, take_par INTEGER,
                                             sort_by_par CHAR VARYING, sort_direction_par CHAR VARYING,
                                             parametros_par CHAR VARYING, valores_par CHAR VARYING) RETURNS TEXT AS
$$
DECLARE
...
total_cuotas_mora := (SELECT "totalCuotasMora"
                                      FROM historico_plan_contrato
                                      WHERE id = cuotas[count]."idHistoricoPlanContrato") + 1;
                total_cuotas_mora_actual := (SELECT "totalCuotasMoraActual"
                                             FROM historico_plan_contrato
                                             WHERE id = cuotas[count]."idHistoricoPlanContrato") + 1;
UPDATE historico_plan_contrato
SET "totalCuotasMora"= total_cuotas_mora
WHERE id = cuotas[count]."idHistoricoPlanContrato";
UPDATE historico_plan_contrato
SET "totalCuotasMoraActual"= total_cuotas_mora_actual
WHERE id = cuotas[count]."idHistoricoPlanContrato";
END IF;
count := count + 1;
END LOOP;
...
begin
    ...
    while(cont <= plazo_par)
        loop
            ...
            if (descuento_primera_cuota > 0 and cont <= 1) then
                abono_capital := abono_capital_par - (abono_capital_par * (descuento_primera_cuota / 100));
                valor_cuota := valor_cuota_par - (valor_cuota_par * (descuento_primera_cuota / 100));
                valor_tasa_administrativa := (valor_cuota - abono_capital) / (1 + iva);
                valor_impuesto := valor_tasa_administrativa * iva;
else
                abono_capital := abono_capital_par;
                valor_cuota := valor_cuota_par;
                valor_tasa_administrativa := valor_tasa_administrativa_par;
                valor_impuesto := valor_impuesto_par;
end if;
...
insert into cuota ("sisActualizado", "sisCreado", "sisHabilitado",
                   ...
                   "valorImpuesto", "abonoCapital", "estaPagado",
                   "estaMora", "dispositivo", "rastreo",
                   "dispositivoEstaPagado", "rastreoEstaPagado", "valorPagadoDispositivo",
                   "valorPagadoRastreo")
values (now(), now(), 'A',
        id_historico_plan_contrato_par, mes_siguiente, cont,
        valor_cuota, valor_pagado_cuota, valor_tasa_administrativa, valor_impuesto,
        abono_capital, esta_pagado,
        esta_mora_par, 0, 0,
        'N', 'N', 0, 0);
cont := cont + 1;
end loop;
...
---- CREAR TABLA CUOTAS ORIGINAL ----
create or replace function crear_tabla_cuotas(fecha_cobro_par date, valor_cuota_par numeric, valor_pagado_cuota_par numeric, valor_tasa_administrativa_par numeric, valor_impuesto_par numeric, abono_capital_par numeric, esta_mora_par character varying, plazo_par integer, cuotas_pagadas_par integer, id_historico_plan_contrato_par integer) returns void
    language plpgsql
as
$$
Declare
...
CREATE OR REPLACE FUNCTION add_filtros(tabla_par CHAR VARYING, and_par BOOLEAN, parametros_par TEXT[],
                                       valores_par TEXT[]) RETURNS TEXT AS
$$
DECLARE
result TEXT;
count  INTEGER;
...
count := 1;
    ...
    IF exist <> 0 THEN
        ...
        WHILE (count <= num)
            LOOP
                IF (count = 1) THEN
                    result := result || ' (' || tabla_par || '.' || quote_ident(parametros_par[count]) || '=' ||
                              quote_literal(valores_par[count]) || ') ';
ELSE
                    result := result || ' AND (' || tabla_par || '.' || quote_ident(parametros_par[count]) || '=' ||
                              quote_literal(valores_par[count]) || ') ';
END IF;
count := count + 1;
END LOOP;
...
END IF;
    ...
    hpc := (SELECT hpc1
            FROM historico_plan_contrato hpc1
            WHERE hpc1."idContrato" = id_contrato
            ORDER BY hpc1."sisCreado" DESC
            LIMIT 1);
...
END IF;
UPDATE contrato
SET "valorFondo" = (hpc."abonosCapitalActual" - licitacion."valor_oferta")
...
CREATE OR REPLACE FUNCTION actualizar_contrato(json_par CHAR VARYING) RETURNS CHAR VARYING AS
$$
DECLARE
contrato               public.contrato;
    nueva_cuota            NUMERIC;
...
END IF;
    ...
    primera_cuota_pagada :=
            (SELECT "estaPagado" FROM cuota WHERE "idHistoricoPlanContrato" = hpc.id AND "numeroCuota" = 1);
...
INSERT INTO contrato ("sisActualizado", "sisCreado", "sisHabilitado", "numeroDeContrato",
...
VALUES (now(), now(), 'A', nc, fecha_inicio, fecha_fin, 0, 0,
...
INSERT INTO historico_plan_contrato ("sisActualizado", "sisCreado",
                                     ...
                                     "valorTasaAdministrativa", "totalTasaAdministrativaCobrada",
                                     "totalCuotasCobradas",
                                     "totalCuotasMoraActual", "totalCuotasMora", "totalMontoCobrado",
                                     "valorDsctoPrimeraCuota",
                                     "totalCobroPrimeraCuota", "valorRecargo", "idContrato", "idPlan",
                                     "numCuotasAbajoHaciaArriba",
                                     "valorADevolver", "inscripcionEstaPagada", "valorPagadoInscripcion",
                                     "totalDispositivoCobrado")
VALUES (now(), now(),
        'A', plan.inscripcion,
...
capital1 := cuota."abonoCapital" * (hpc."totalCuotasCobradas" - 1);
    ...
    tasa_administrativa1 := (cuota."valorTasaAdministrativa" * (hpc."totalCuotasCobradas" - 1)) +
                            (cuota."valorImpuesto" * (hpc."totalCuotasCobradas" - 1));
    tasa_administrativa2 := (tasa_administrativa * numero_cuotas_pasar) + (impuesto * numero_cuotas_pasar) +
                            tasa_administrativa_restante;
...
INSERT INTO contrato ("sisActualizado", "sisCreado", "sisHabilitado", "numeroDeContrato",
                      ...
                      "fechaIniciaCobro", "estado", "plazoMesSeleccionado", version, "cuotaActual",
                      "tipoDocumentoIdentidad", "documentoIdentidad", "planSeleccionado", "precioPlanSeleccionado",
                      "nombresCliente", "apellidosCliente", "nombreGrupo", "dsctoRecargo", "idClienteEnGrupo",
                      "idVendedor", "valorFondo", "cuotaACobrar")
VALUES (now(), now(), 'A', nc,
...
BEGIN
    ...
    IF (existe_grupo = 0) THEN
        INSERT INTO grupo ("sisActualizado", "sisCreado", "sisHabilitado", "nombreGrupo", "sumatoriaMontoMeta",
                           "totalContratosPermitidos", "totalContratosUsados")
        values (now(), now(), 'A', 1, 0, 144, 1)
        RETURNING id, "nombreGrupo" INTO id_grupo, ng;
ELSE
        ...
        existe_cliente_en_grupo := (SELECT count(*)
                                    FROM cliente_en_grupo ceg
                                    WHERE ceg."idCliente" = id_cliente AND ceg."idGrupo" = gp.id);

        IF (existe_cliente_en_grupo = 0) THEN
UPDATE grupo
SET "sisActualizado"=now(), "totalContratosUsados"=(gp."totalContratosUsados" + 1)
WHERE id = gp.id;
id_grupo := gp.id;
            ng := gp."nombreGrupo";
ELSE
            INSERT INTO grupo ("sisActualizado", "sisCreado", "sisHabilitado", "nombreGrupo", "sumatoriaMontoMeta",
                               "totalContratosPermitidos", "totalContratosUsados")
            values (now(), now(), 'A', (gp."nombreGrupo" + 1), 0, 144, 1)
            RETURNING id, "nombreGrupo" INTO id_grupo, ng;
END IF;
END IF;

    -- crear cliente en grupo
INSERT INTO cliente_en_grupo ("sisActualizado", "sisCreado", "sisHabilitado", "idCliente", "idGrupo")
values (now(), now(), 'A', id_cliente, id_grupo)
    RETURNING id INTO id_cg;
...
INSERT INTO contrato ("sisActualizado", "sisCreado", "sisHabilitado", "numeroDeContrato",
                      ...
                      "nombresCliente", "apellidosCliente", "nombreGrupo", "dsctoRecargo", "idClienteEnGrupo",
                      "idVendedor", "valorFondo", "cuotaACobrar")
VALUES (now(), now(), 'A', nc, fecha_inicio, fecha_fin, dscto_ins, dscto_pc,
...
total_cobro_inscripcion := plan.inscripcion - (plan.inscripcion * (dscto_ins / 100));
    ...
    -- Insertar nuevo historico plan contrato
INSERT INTO historico_plan_contrato ("sisActualizado", "sisCreado", "sisHabilitado", "totalInscripcionPlan",
                                     "valorDsctoInscripcion", "totalCobroInscripcion", "capitalTotal",
                                     "capitalPorRefinanciamiento", "abonosCapitalActual", "saldoCapital",
                                     "valorTasaAdministrativa", "totalTasaAdministrativaCobrada",
                                     "totalCuotasCobradas",
                                     ...
                                     "valorDsctoPrimeraCuota",
                                     "totalCobroPrimeraCuota", "valorRecargo", "idContrato", "idPlan",
                                     "numCuotasAbajoHaciaArriba", "valorADevolver", "inscripcionEstaPagada",
                                     "valorPagadoInscripcion", "totalDispositivoCobrado")
VALUES (now(), now(), 1, plan.inscripcion,
...
VALUES (now(), now(), 'A', nc, fecha_inicio, fecha_fin, 0, 0,
        observacion, fic, 'REG', pms, (contrato.version + 1), nueva_cuota,
        contrato."tipoDocumentoIdentidad", contrato."documentoIdentidad", plan.modelo, plan.precio,
        contrato."nombresCliente", contrato."apellidosCliente", contrato."nombreGrupo", 0,
        contrato."idClienteEnGrupo", id_vendedor, 0, 0)
    RETURNING id INTO id_contrato;
...
INSERT INTO historico_plan_contrato ("sisActualizado", "sisCreado", "sisHabilitado", "totalInscripcionPlan",
                                     ...
                                     "valorADevolver", "inscripcionEstaPagada", "valorPagadoInscripcion",
                                     "totalDispositivoCobrado")
VALUES (now(), now(), 'A', plan.inscripcion,
        0, valor_nueva_incripcion, plan.precio,
        0, abonos_capital_actual, saldo_capital,
        tasa_administrativa, total_tasa_administrativa_cobrada, numero_cuotas_pasar,
        0, 0, total_monto_cobrado, 0, 0,
...
BEGIN
    ...
    numero_cuotas_faltantes_refinanciamiento :=
            (SELECT round(hpc."totalCuotasMoraActual" - (hpc."totalCuotasMoraActual" * 0.25)));
...
INSERT INTO contrato ("sisActualizado", "sisCreado", "sisHabilitado", "numeroDeContrato",
                      ...
                      "nombresCliente", "apellidosCliente", "nombreGrupo", "dsctoRecargo", "idClienteEnGrupo",
                      "idVendedor", "valorFondo", "cuotaACobrar")
VALUES (now(), contrato."sisCreado", 'A', contrato."numeroDeContrato",
        contrato."fechaInicio", contrato."fechaFin", contrato."dsctoInscripcion", contrato."dsctoPrimeraCuota",
        contrato.observacion,
        contrato."fechaIniciaCobro", 'ERP', contrato."plazoMesSeleccionado", (contrato.version + 1),
        contrato."cuotaActual",
        contrato."tipoDocumentoIdentidad", contrato."documentoIdentidad", plan.modelo, plan.precio,
        contrato."nombresCliente", contrato."apellidosCliente", contrato."nombreGrupo", contrato."dsctoRecargo",
        contrato."idClienteEnGrupo", contrato."idVendedor", 0, 0)
    RETURNING id INTO id_contrato;

-- sacar los resto de las cuotas que no se pagaron
resto_inicio := (SELECT "valorPagadoCuota"
                     FROM cuota
                     WHERE "idHistoricoPlanContrato" = hpc.id
                       AND "numeroCuota" = (hpc."totalCuotasCobradas" + 1));
    resto_final := (SELECT "valorPagadoCuota"
                    FROM cuota
                    WHERE "idHistoricoPlanContrato" = hpc.id
                      AND "numeroCuota" = (pms - hpc."numCuotasAbajoHaciaArriba" - 1));


    -- crear nuevo historico plan contrato
    numero_cuotas_pasar :=
            hpc."totalCuotasCobradas" + numero_cuotas_pagar_refinanciamiento + numero_cuotas_faltantes_refinanciamiento;
    ...
    saldo_capital := (nueva_cuota * (numero_cuotas_restantes_sin_mora - 2)) + (nueva_cuota - resto_inicio) +
                     (nueva_cuota - resto_final);
...
INSERT INTO historico_plan_contrato ("sisActualizado", "sisCreado", "sisHabilitado", "totalInscripcionPlan",
                                     ...
                                     "valorADevolver", "inscripcionEstaPagada", "valorPagadoInscripcion",
                                     "totalDispositivoCobrado")
VALUES (now(), hpc."sisCreado", 'A', hpc."totalInscripcionPlan",
...
PERFORM public.cuotas_refinanciamiento(cuota, plan, nueva_cuota, nuevo_abono_capital, tasa_administrativa, impuesto,
                                           resto_inicio, numero_cuotas_pasar, resto_final, pms, id_historico);

INSERT INTO public.refinanciamiento("sisActualizado", "sisCreado", "sisHabilitado",
...
VALUES (now(), now(), 'A', nc, fecha_inicio, fecha_fin, 0, 0,
        ...
        contrato1."nombresCliente", contrato1."apellidosCliente", nombre_grupo, dscto_recargo, id_cliente_en_grupo,
        id_vendedor, 0)
...
capital1 := cuota1."abonoCapital" * (hpc1."totalCuotasCobradas" - 1);
    ...
    tasa_administrativa1 := (cuota1."valorTasaAdministrativa" * (hpc1."totalCuotasCobradas" - 1)) +
                            (cuota1."valorImpuesto" * (hpc1."totalCuotasCobradas" - 1));
    tasa_administrativa2 := (cuota2."valorTasaAdministrativa" * (hpc2."totalCuotasCobradas" - 1)) +
                            (cuota2."valorImpuesto" * (hpc2."totalCuotasCobradas" - 1));
    tasa_administrativa3 := (tasa_administrativa * numero_cuotas_pasar) + (impuesto * numero_cuotas_pasar) +
                            tasa_administrativa_restante;
...
count := 0;
    ...
    num := (SELECT COUNT(*)
            FROM cuota
            WHERE "idHistoricoPlanContrato" = id_historico_plan_contrato
              AND "estaPagado" = 'N'
              AND "estaMora" = 'N');
    contrato := (SELECT c
                 FROM contrato c
                          INNER JOIN historico_plan_contrato hpc ON hpc.id = c.id
                 WHERE hpc.id = id_historico_plan_contrato);
    IF num <> 0 THEN
        WHILE (count < num)
            LOOP
...
UPDATE cuota
SET "fechaCobro"= next_month
WHERE "idHistoricoPlanContrato" = id_historico_plan_contrato
  AND "numeroCuota" = num_cuota;
count := count + 1;
END LOOP;
END IF;
...
END IF;
    ...
    hpc := (SELECT hpc1
            ...
            LIMIT 1);
    num_cuotas :=
            (SELECT count(cuo) FROM cuota cuo WHERE cuo."idHistoricoPlanContrato" = hpc.id AND cuo."estaPagado" = 'N');
...
UPDATE historico_plan_contrato
SET "capitalALiquidar"            = capital_a_liquidar,
    "tasaAdministrativaALiquidar" = tasa_administrativa_a_liquidar
WHERE id = hpc.id;
...
SET rastreo = 0
WHERE "idHistoricoPlanContrato" = hpc.id
  AND "numeroCuota" BETWEEN con."cuotaACobrar" AND num_cuotas;
...
count := 1;
    ...
    historico_plan_contrato := (SELECT hpc
                                FROM historico_plan_contrato hpc
                                WHERE hpc."idContrato" = id_contrato
                                ORDER BY hpc.id DESC
                                LIMIT 1);
    ------------- Actualizacion de la primera cuota con dispositivo y rastreo  -------------
    cuo := (SELECT cuo11
            FROM cuota cuo11
            WHERE cuo11."idHistoricoPlanContrato" = historico_plan_contrato.id
              AND cuo11."estaPagado" = 'N'
            ORDER BY cuo11."numeroCuota" ASC
            LIMIT 1);
...
WHILE (count < num)
        LOOP
            next_cuota := (SELECT cuo1
                           FROM cuota cuo1
                           WHERE cuo1."idHistoricoPlanContrato" = historico_plan_contrato.id
                             AND cuo1."estaPagado" = 'N'
                             AND cuo1."numeroCuota" = cuo."numeroCuota" + count);
            new_valor_cuota := next_cuota."valorCuota" + cg.dispositivo + cg.rastreo;
UPDATE cuota
SET "valorCuota" = new_valor_cuota,
    dispositivo=cg.dispositivo,
    rastreo=cg.rastreo
WHERE "id" = next_cuota.id;
count := count + 1;
END LOOP;
    ---------- Actualizar las demas cuotas con el rastreo  -------------
    last_cuota := (SELECT cuo2
                   FROM cuota cuo2
                   WHERE cuo2."idHistoricoPlanContrato" = historico_plan_contrato.id
                     AND cuo2."estaPagado" = 'N'
                   ORDER BY cuo2."numeroCuota" DESC
                   LIMIT 1);
    num := last_cuota."numeroCuota";
count := next_cuota."numeroCuota" + 1;
    WHILE (count <= num)
        LOOP
            next_cuota := (SELECT cuo3
                           FROM cuota cuo3
                           WHERE cuo3."idHistoricoPlanContrato" = historico_plan_contrato.id
                             AND cuo3."numeroCuota" = count);
...
INSERT INTO cobro ("sisActualizado", "sisCreado", "sisHabilitado",
...
INSERT INTO pago ("sisActualizado", "sisCreado", "sisHabilitado",
...
INSERT INTO detalle_pago ("sisActualizado", "sisCreado", "sisHabilitado",
                          "valor",
...
---###### Generacion de asientos contables
                perform public.asiento_cobros(id_contrato, id_cobro, c."estado", peri.id, peri."anio",
                                              peri."fechaInicio",
                                              c."nombresCliente"::text, c."cuotaACobrar", fecha_deposito, id_pago,
                                              w_ivaPorcentaje::numeric(12, 2), banco_cuenta_bancaria);
    ...
    IF (num3 <> 0) THEN
        WHILE (count3 < num3)
            LOOP
                ...
                cuo := (SELECT cuo1
                        FROM cuota cuo1
                        WHERE cuo1."numeroCuota" = numero_cuota AND cuo1."idHistoricoPlanContrato" = hpc.id);
                total_dispositivo_cobrado :=
                        (SELECT "totalDispositivoCobrado" FROM historico_plan_contrato WHERE id = hpc.id);
...
END IF;
                ...
                IF (tipo_cobro = 'CA') THEN
                    ...
                    total_tasa_administrativa_cobrada :=
                            coalesce(total_tasa_administrativa_cobrada, 0) + coalesce(a_cobrar, 0);
...
UPDATE cuota
SET "valorPagadoCuota"= (cuo."valorPagadoCuota" + a_cobrar)
WHERE "numeroCuota" = numero_cuota
  AND "idHistoricoPlanContrato" = hpc.id;
END IF;
...
END IF;
UPDATE cuota
SET "dispositivoEstaPagado" = estado_pago_dispositivo,
    "valorPagadoDispositivo"= monto_pagado_dispositivo
WHERE "numeroCuota" = numero_cuota
  AND "idHistoricoPlanContrato" = hpc.id;
END IF;
...
END IF;
UPDATE cuota
SET "rastreoEstaPagado"     = estado_pago_rastreo,
    "valorPagadoDispositivo"= monto_pagado_rastreo
WHERE "numeroCuota" = numero_cuota
  AND "idHistoricoPlanContrato" = hpc.id;
END IF;
...
END IF;
                    ...
                    perform public.asiento_cobros_externo(id_contrato, id_cobro, c.estado, peri.id, peri."anio",
                                                          peri."fechaInicio",
                                                          c."nombresCliente", numero_cuota, "fechaDeposito", id_pago,
                                                          w_ivaPorcentaje, a_cobrar, tipo_cobro, banco_cuenta_bancaria);
...
UPDATE cuota
SET "estaPagado" = 'S'
WHERE "numeroCuota" = numero_cuota AND "idHistoricoPlanContrato" = hpc.id;
...
raise notice 'value of id_contrato : %', id_contrato;
                ...
                ELSEIF (coalesce(monto_pagado_cuota, 0) < (valor_cuota - coalesce(hpc."valorDsctoPrimeraCuota", 0)) AND
                        numero_cuota = 1) THEN
                    -- RETURN '{"correcto":false, "message":" entro 2"}',monto_pagado_cuota::text ;

                    total_cobro_primera_cuota :=
                            coalesce(total_cobro_primera_cuota, 0) + coalesce(monto_pagado_cuota, 0);
                ...
                ELSEIF (
                    coalesce(monto_pagado_cuota, 0) = (c."cuotaActual" - coalesce(hpc."valorDsctoPrimeraCuota", 0)) AND
                    numero_cuota = 1) THEN
                    raise notice 'value of monto_pagado_cuota : %', monto_pagado_cuota;
                    raise notice 'value of total_cuotas_cobradas : %', total_cuotas_cobradas;
...
total_cobro_primera_cuota := coalesce(monto_pagado_cuota, 0);
                    if hpc."numCuotasAbajoHaciaArriba" = 0 then
                        ...
                        total_monto_cobrado := coalesce(total_monto_cobrado, 0) + coalesce(total_cobro_primera_cuota, 0)
                            - coalesce(hpc."valorTasaAdministrativa", 0) - round(
                                                       (coalesce(hpc."valorTasaAdministrativa", 0) * (w_ivaPorcentaje / 100)),
                                                       2);
...
UPDATE historico_plan_contrato
SET "valorPagadoInscripcion"         = valor_pagado_inscripcion,
...
END IF;
    ...
    id_grupo := (SELECT cg."idGrupo"
                 FROM cliente_en_grupo cg
                          INNER JOIN contrato c1 ON c1."idClienteEnGrupo" = cg."id"
                 WHERE c1.id = id_contrato);
...
create or replace function actualiza_saldo_ctacble(w_cuentacontable integer, nombrecolumna text,
                                                   valorcreditodebito numeric) returns void
    language plpgsql
as
$$
declare
...
create or replace function asiento_cobros(id_contrato integer, id_cobro integer, estado_contrato character varying,
                                          idperiodo integer, anio integer, fechainicio date,
                                          nombrescliente character varying, cuotaacobrar integer, fecha_deposito date,
                                          id_pago integer, w_ivaporcentaje numeric,
                                          banco_cuenta_bancaria character varying) returns void
    language plpgsql
as
$$
declare
...
begin
select trim("codigoCuentaContable")
into w_codigoCuentaContableIns
...
where "nombre_cuenta" = 'INGRESO DE INSCRIPCION AUTOS';
            ...
            perform asiento_cobrosreg(id_contrato, id_cobro, estado_contrato, idperiodo, anio, fechaInicio,
                                      nombrescliente, cuotaacobrar, fecha_deposito, id_pago, nombreColumna,
                                      w_mesPeriodo,
...
into w_valorAC, w_idItemCobroPagoAC
from detalle_pago
where tipo = 'AC'
  and "idPago" = id_pago;
end;
...
begin
select valor, "idItemCobroPago"
into w_valorCA, w_idItemCobroPagoCA
from detalle_pago
where tipo = 'CA'
  and "idPago" = id_pago;
end;
...
update contrato set "estado"='EPR' where "id" = id_contrato;
        ...
        w_valorAC := 0;
begin
select sum(valor), max("idItemCobroPago")
into w_valorAC, w_idItemCobroPagoAC
from detalle_pago
where tipo = 'AC'
  and "idPago" = id_pago;
...
begin
select sum(valor), max("idItemCobroPago")
into w_valorDIS, w_idItemCobroPagoDIS
from detalle_pago
where tipo = 'D'
  and "idPago" = id_pago;
end;
...
begin
select sum(valor), max("idItemCobroPago")
into w_valorRAS, w_idItemCobroPagoRAS
from detalle_pago
where tipo = 'R'
  and "idPago" = id_pago;
end;
        ...
        perform asiento_cobrosadj(id_contrato, id_cobro, estado_contrato, idperiodo, anio, fechainicio,
                                  ...
                                  w_valorDIS, w_idItemCobroPagoDIS, w_valorRAS, w_idItemCobroPagoRAS,
...
create or replace function asiento_cobrosadj(id_contrato integer, id_cobro integer, estado_contrato character varying,
                                             w_idperiodo integer, w_anio integer, w_fechainicio date,
                                             w_nombrescliente character varying, w_cuotaacobrar integer,
                                             w_fechadeposito date, id_pago integer, nombrecolumna character varying,
                                             w_mesperiodo character varying, w_ivaporcentaje numeric, w_valorac numeric,
                                             w_iditemcobropago integer, w_valorca numeric, w_iditemcobropagoca integer,
                                             w_valordis numeric, w_iditemcobropagodis integer, w_valorras numeric,
                                             w_iditemcobropagoras integer,
                                             banco_cuenta_bancaria character varying) returns void
    language plpgsql
as
...
where "idPeriodoContable" = w_idperiodo
  and "codigoCuentaContable" in (select trim("codigoCuentaContable")
                                 from item_cobro_pago
                                 where "nombre_cuenta" =
...
begin
INSERT INTO public.asiento_contable_cabecera("sisActualizado", "sisCreado", "sisHabilitado", anio,
                                             "asientoCerrado", beneficiario,
                                             descripcion, fecha, "mesPeriodo", "tipoAsientoContable",
                                             "tipoTransaccion",
                                             "totalCredito", "totalDebito", "totalSaldoActualFecha",
                                             "idCuentaContable")
VALUES (now(), now(), 'A', w_anio, 'N', w_nombresCliente,
        'Pago cuota Nro.' || w_cuotaACobrar, w_fechaDeposito, w_mesPeriodo, 'E', w_TipoTransaccion,
        0, 0, 0,
        w_idCuentaContableCxC) ---cuenta contable CUENTAS Y DOCUMENTOS POR COBRAR COMERCIALES CORRIENTES
...
begin
INSERT INTO public.transaccion_asiento_contable("sisActualizado", "sisCreado",
                                                "sisHabilitado", detalle,
                                                "valorCredito", "valorDebito",
                                                "idAsientoContableCabecera",
                                                "idCuentaContable")
VALUES (now(), now(), 'A', 'CUENTAS Y DOCUMENTOS POR COBRAR COMERCIALES CORRIENTES',
        0, (w_valorCA), id_AsientoContableCabecera,
        w_idCuentaContableCxC) --- 2 es valor quemado para esta cuenta
    RETURNING id INTO id_transaccionAsientoContable;
end;

begin
                            perform public.actualiza_saldo_ctacble(w_idCuentaContableCxC, nombreColumna || 'Debito"',
                                                                   (coalesce(w_valorCA, 0)));
end;
...
VALUES (now(), now(), 'A', 'IVA POR PAGAR',
        (w_valor_iva), 0, id_AsientoContableCabecera,
        w_idCuentaContableIva) --- 3 es valor quemado para esta cuenta
    RETURNING id INTO id_transaccionAsientoContable;
end;

begin
                            perform public.actualiza_saldo_ctacble(w_idCuentaContableIva, nombreColumna || 'Credito"',
                                                                   coalesce(w_valor_iva, 0));
end;
...
VALUES (now(), now(), 'A', 'INGRESOS POR ADMINISTRACION AUTOS',
        w_valorTasaAdministrativa, 0, id_AsientoContableCabecera,
        w_idCuentaContableIAA) --- 3 es valor quemado para esta cuenta
    RETURNING id INTO id_transaccionAsientoContable;
end;

begin
                            perform public.actualiza_saldo_ctacble(w_idCuentaContableIAA, nombreColumna || 'Credito"',
                                                                   coalesce(w_valorTasaAdministrativa, 0));
end;
end if;
...
INSERT INTO public.transaccion_asiento_contable("sisActualizado", "sisCreado",
                                                ...
                                                "idCuentaContable")
VALUES (now(), now(), 'A', 'CUENTAS Y DOCUMENTOS POR COBRAR COMERCIALES CORRIENTES',
        0, (w_valorCA + w_valorRAS + w_valorDIS), id_AsientoContableCabecera,
        w_idCuentaContableCxC) --- 2 es valor quemado para esta cuenta
    RETURNING id INTO id_transaccionAsientoContable;
end;
...
begin
                            perform public.actualiza_saldo_ctacble(w_idCuentaContableCxC, nombreColumna || 'Debito"',
                                                                   (coalesce(w_valorCA, 0) + coalesce(w_valorRAS, 0) +
                                                                    coalesce(w_valorDIS, 0)));
end;
...
INSERT INTO public.transaccion_asiento_contable("sisActualizado", "sisCreado",
                                                ...
                                                "idCuentaContable")
VALUES (now(), now(), 'A', 'IVA POR PAGAR',
        (w_valor_iva + w_valor_ivaRAS + w_valor_ivaDIS), 0, id_AsientoContableCabecera,
        w_idCuentaContableIva) --- 3 es valor quemado para esta cuenta
    RETURNING id INTO id_transaccionAsientoContable;
end;
...
begin
                            perform public.actualiza_saldo_ctacble(w_idCuentaContableIva, nombreColumna || 'Credito"',
                                                                   (coalesce(w_valor_iva, 0) +
                                                                    coalesce(w_valor_ivaRAS, 0) +
                                                                    coalesce(w_valor_ivaDIS, 0)));
end;
...
begin
                                perform public.actualiza_saldo_ctacble(w_idCuentaContableIAA,
                                                                       nombreColumna || 'Credito"',
                                                                       coalesce(w_valorTasaAdministrativa, 0));
end;
end if;
                        ----------------
...
VALUES (now(), now(), 'A', 'SERVICIO DE SEGURIDAD INTERNO DISPOSITIVO',
        w_valorTasaAdministrativaDIS, 0, id_AsientoContableCabecera,
        w_idCuentaContableSSD) --- 6 es valor quemado para esta cuenta
    RETURNING id INTO id_transaccionAsientoContable;
end;

begin
                                perform public.actualiza_saldo_ctacble(w_idCuentaContableSSD,
                                                                       nombreColumna || 'Credito"',
                                                                       coalesce(w_valorTasaAdministrativaDIS, 0));
end;
end if;
...
VALUES (now(), now(), 'A', 'SISTEMA DE SEGURIDAD INTERNO MONITOREO',
        w_valorTasaAdministrativaRAS, 0, id_AsientoContableCabecera,
        w_idCuentaContableSSR) --- 7 es valor quemado para esta cuenta
    RETURNING id INTO id_transaccionAsientoContable;
end;

begin
                                perform public.actualiza_saldo_ctacble(w_idCuentaContableSSR,
                                                                       nombreColumna || 'Credito"',
                                                                       coalesce(w_valorTasaAdministrativaRAS, 0));
end;
end if;
                        ----------------
                        -----------------
...
begin
INSERT INTO public.transaccion_asiento_contable("sisActualizado", "sisCreado", "sisHabilitado",
                                                detalle,
                                                "valorCredito", "valorDebito",
                                                "idAsientoContableCabecera", "idCuentaContable")
VALUES (now(), now(), 'A', 'BANCO ' || banco_cuenta_bancaria,
        0, (w_valorCA + w_valorAC + w_valorRAS + w_valorDIS), id_AsientoContableCabecera,
        w_idCuentaContableBco)
    RETURNING id INTO id_transaccionAsientoContable;
end;

begin
                    perform public.actualiza_saldo_ctacble(w_idCuentaContableBco, nombreColumna || 'Debito"',
                                                           (coalesce(w_valorAC, 0) + coalesce(w_valorCA, 0)));
end;
...
INSERT INTO public.transaccion_asiento_contable("sisActualizado", "sisCreado", "sisHabilitado",
                                                ...
                                                "idAsientoContableCabecera", "idCuentaContable")
VALUES (now(), now(), 'A', 'CUENTAS Y DOCUMENTOS POR COBRAR COMERCIALES CORRIENTES',
        (w_valorCA + w_valorRAS + w_valorDIS), 0, id_AsientoContableCabecera,
        w_idCuentaContableCxC) --- 2 es valor quemado para esta cuenta
    RETURNING id INTO id_transaccionAsientoContable;
end;
...
begin
                        perform public.actualiza_saldo_ctacble(w_idCuentaContableCxC, nombreColumna || 'Credito"',
                                                               (coalesce(w_valorCA, 0) + coalesce(w_valorRAS, 0) +
                                                                coalesce(w_valorDIS, 0)));
end;
end if;
...
where "codigoCuentaContable"
          in (select trim("codigoCuentaContable")
              from item_cobro_pago
...
begin
                    perform public.actualiza_saldo_ctacble(w_idCuentaContable, nombreColumna || 'Credito"',
                                                           coalesce(w_valorAC, 0));
end;
...
create or replace function asiento_cobrosreg(id_contrato integer, id_cobro integer, estado_contrato character varying,
                                             w_idperiodo integer, w_anio integer, w_fechainicio date,
                                             w_nombrescliente character varying, w_cuotaacobrar integer,
                                             w_fechadeposito date, id_pago integer, nombrecolumna character varying,
                                             w_mesperiodo character varying, w_ivaporcentaje numeric, w_valor numeric,
                                             w_iditemcobropago integer, w_codigocuentacontableins character varying,
                                             banco_cuenta_bancaria character varying, w_valorac numeric,
                                             w_valorca numeric, w_tipo character varying) returns void
    language plpgsql
as
$$
declare
...
INSERT INTO public.transaccion_asiento_contable("sisActualizado", "sisCreado", "sisHabilitado",
                                                ...
                                                "idAsientoContableCabecera", "idCuentaContable")
VALUES (now(), now(), 'A', 'CUENTAS Y DOCUMENTOS POR COBRAR COMERCIALES CORRIENTES',
        0, w_valor, id_AsientoContableCabecera,
        w_idCuentaContableCxC) --- 2 es valor quemado para esta cuenta
    RETURNING id INTO id_transaccionAsientoContable;
end;
begin
...
from cuenta_contable
where "idPeriodoContable" = w_idperiodo
  and "codigoCuentaContable" = w_codigoCuentaContableIns;
end;
                    raise notice '###########';
...
begin
                        perform public.actualiza_saldo_ctacble(w_idCuentaContableInscri, nombreColumna || 'Debito"',
                                                               w_diferencia);
end;
...
INSERT INTO public.transaccion_asiento_contable("sisActualizado", "sisCreado", "sisHabilitado",
                                                ...
                                                "idAsientoContableCabecera", "idCuentaContable")
VALUES (now(), now(), 'A', 'CUENTAS Y DOCUMENTOS POR COBRAR COMERCIALES CORRIENTES',
        w_valorCA, 0, id_AsientoContableCabecera,
        w_idCuentaContableCxC) --- 2 es valor quemado para esta cuenta
    RETURNING id INTO id_transaccionAsientoContable;
end;
...
VALUES (now(), now(), 'A', 'NO ADJUDICADO AUTOS',
        w_valorAC, 0, id_AsientoContableCabecera,
        w_idCuentaContableNoAdjudi) --- 2 es valor quemado para esta cuenta
    RETURNING id INTO id_transaccionAsientoContable;
end;
...
begin
                        perform public.actualiza_saldo_ctacble(w_idCuentaContableNoAdjudi, nombreColumna || 'Credito"',
                                                               w_valorAC);
end;
...
INSERT INTO public.transaccion_asiento_contable("sisActualizado", "sisCreado", "sisHabilitado",
                                                ...
                                                "idAsientoContableCabecera", "idCuentaContable")
VALUES (now(), now(), 'A', 'CUENTAS Y DOCUMENTOS POR COBRAR COMERCIALES CORRIENTES',
        w_valor, 0, id_AsientoContableCabecera,
        w_idCuentaContableCxC) --- 2 es valor quemado para esta cuenta
    RETURNING id INTO id_transaccionAsientoContable;
end;
...
BEGIN
    ...
    con := (SELECT c1
            FROM cobro
                     INNER JOIN contrato c1 ON c1.id = cobro."idContrato"
            WHERE cobro.id = jsonbpar.id_cobro);
        ...
        WHILE (num >= count)
            LOOP
                ...
                IF (tipo_cobro = 'I') THEN
                    ...
                    valor_pagado_inscripcion := valor_pagado_inscripcion - a_cobrar;
UPDATE historico_plan_contrato
SET "inscripcionEstaPagada"  = inscripcion_esta_pagada,
...
UPDATE cuota
SET "valorPagadoDispositivo"= monto_pagado_cuota
WHERE "numeroCuota" = numero_cuota
  AND "idHistoricoPlanContrato" = hpc.id;
END IF;
                IF (tipo_cobro = 'R') THEN
...
UPDATE cuota
SET "valorPagadoRastreo"= monto_pagado_cuota
WHERE "numeroCuota" = numero_cuota
  AND "idHistoricoPlanContrato" = hpc.id;
END IF;
                IF (tipo_cobro = 'CAD') THEN
...
BEGIN
    ...
    id_hpc := (SELECT h1.id
               FROM historico_plan_contrato h1
               WHERE h1."idContrato" = id_contrato
               ORDER BY h1.id DESC
               LIMIT 1);
UPDATE cuota
SET "estadoPago"='N'
...
UPDATE cuota
SET "dispositivoEstaPagado" = 'N'
WHERE "idHistoricoPlanContrato" = id_hpc
  AND dispositivo > "valorPagadoDispositivo";
UPDATE cuota
SET "rastreoEstaPagado" = 'N'
WHERE "idHistoricoPlanContrato" = id_hpc
  AND rastreo > "valorPagadoRastreo";
...
END IF;
    ...
    total_cobro_primera_cuota := (SELECT c3."valorPagadoCuota"
                                  FROM cuota c3
                                  WHERE c3."idHistoricoPlanContrato" = id_hpc AND c3."numeroCuota" = 1);
    total_dispositivo_cobrado := (SELECT count(*)
                                  FROM cuota c4
                                  WHERE c4."idHistoricoPlanContrato" = id_hpc AND c4."dispositivoEstaPagado" = 'S');
UPDATE contrato SET "cuotaACobrar" = last_cuota WHERE id = id_contrato;
UPDATE historico_plan_contrato
SET "abonoCapitalActual"             = abonos_capital_actual,
    "saldoCapital"                   = saldo_capital,
    "totalTasaAdministrativaCobrada" = total_tasa_administrativa_cobrada,
    "totalCuotasCobradas"            = total_cuotas_cobradas,
    "totalMontoCobrado"              = total_monto_cobrado,
...
create or replace function asiento_cobros_externo(id_contrato integer, id_cobro integer,
                                                  estado_contrato character varying, idperiodo integer, anio integer,
                                                  fechainicio date, nombrescliente character varying,
                                                  cuotaacobrar integer, fecha_deposito date, id_pago integer,
                                                  w_ivaporcentaje numeric, w_valor_adj numeric,
                                                  w_tipo_cobro character varying,
                                                  banco_cuenta_bancaria character varying) returns void
    language plpgsql
as
$$
...
where "nombre_cuenta" = 'INGRESOS POR ADJUDICACION AUTOS';
        ...
        perform asiento_cobrosreg(id_contrato, id_cobro, estado_contrato, idperiodo, anio, fechaInicio,
                                  nombrescliente, cuotaacobrar, fecha_deposito, id_pago, nombreColumna, w_mesPeriodo,
                                  w_ivaporcentaje, w_valor_adj, w_idItemCobroPago, w_codigoCuentaContableIns,
                                  banco_cuenta_bancaria);
end if;
...
---- OBTENER MES ---
create or replace function obtener_mes(numero integer) returns character varying
    language plpgsql
as
$$
DECLARE
...
num_factura := (select nextval('factura_no_seq'));
    ...
    factura :=
            jsonb_build_object('agenteRetencion', false, 'amCodigo', '1', 'codigoPorcentaje', '2', 'direccionComprador',
                               'PRUEBAS', 'direccionMatriz', 'PRUEBAS', 'establecimientoEmpresa',
                               codigo_establecimiento, 'facCodIva', '2',
                               'facDescuento', 0.0000, 'facFecha',
                               to_char(current_timestamp, 'YYYY-MM-DD"T"HH24:MI:SSOF:TZM'), 'facIva', 0.00, 'facMoneda',
                               'DOLAR',
                               ...
                               'facTotalBaseGravada', 0.00, 'facUnidaTiempo', 'dias', 'facValorIce', 0.00, 'grabaICE',
                               false,
                               ...
                               'tipoIdentificacionComprador', tipo_identificacion, 'codigoFormaPago', '01',
...
RAISE NOTICE 'id pago % ', id_pago;
                det_pagos :=
                        (select d
                         from detalle_pago d
                         where d."idPago" = id_pago and d.tipo = (item ->> 'tipo')::varchar
                         limit 1);
            ...
            total_iva := coalesce(total_iva,0) + coalesce(valor_iva, 0);*/
                ...
                det_factura := jsonb_build_object('detTarifa', iva, 'detIva', iva_item, 'detSubtotal', subtotal_item,
                                                  'detSubtotaldescuento', subtotal_item, 'codigoProducto',
                                                  trim(to_char(idx, '00000')),
                                                  'descripcionProducto', item ->> 'descripcion', 'detCantidad', 1.00,
                                                  'detCantpordescuento',
                                                  0.00000,
...
values (CURRENT_TIMESTAMP, 'A', empresa."direccionEmpresa", CURRENT_DATE, contrato."documentoIdentidad", total,
        ...
        empresa."nombreComercial", num_documento, pto_emision, empresa."razonSocial", empresa."rucEmpresa",
        factura);
return;
END
...
---- ASIENTO COBROS EPR ---
create or replace function asiento_cobrosepr(id_contrato integer, id_cobro integer, estado_contrato character varying,
                                             w_idperiodo integer, w_anio integer, w_fechainicio date,
                                             w_nombrescliente character varying, w_cuotaacobrar integer,
                                             w_fechadeposito date, w_idcobro integer, nombrecolumna character varying,
                                             w_mesperiodo character varying,
                                             banco_cuenta_bancaria character varying) returns void
    language plpgsql
as
$$
...
FROM CUOTA
where "idHistoricoPlanContrato" = id_contrato
  and "numeroCuota" = w_cuotaACobrar;
end;
...
VALUES (now(), now(), 'A', 'INGRESOS POR ADMINISTRACION AUTOS',
        w_valorTasaAdministrativa, 0, id_AsientoContableCabecera,
        5) --- 3 es valor quemado para esta cuenta
    RETURNING id INTO id_transaccionAsientoContable;
end;
...
from detalle_pago
where tipo = 'AC'
  and "idPago" in (select pago.id
                   from pago,
                        cobro
                   where pago."idCobro" = cobro.id
                     and cobro."idContrato" = id_contrato
                     and cobro.id = id_cobro); -- este cobro.id  y idContrato vienen de parametro
end;
...
from cuenta_contable
where id in (select trim("idCuentaContable")
             from item_cobro_pago
...
VALUES (now(), now(), 'A', 'BANCOS ' || banco_cuenta_bancaria,
        0, w_valor, id_AsientoContableCabecera,
        4) --- 4 codigo de idcuenta es valor quemado para esta cuenta
    RETURNING id INTO id_transaccionAsientoContable;
end;
...
create or replace function calcular_decimo_cuarto(fecha_inicio date, fecha_fin date, id_trabajador integer,
                                                  id_periodo integer, tipo character varying) returns text
    language plpgsql
as
$$
DECLARE
...
id_rol_pago_dic := (select rp.id
                        from periodo_laboral pl
                                 inner join rol_pago rp on pl.id = rp."idPeriodoLaboral"
                        where pl.mes = 'DIC'
                          and pl.anio = anio_periodo);
...
valor_real := (sueldo_basico * dias_laborales_anio) / 360;
    ...
    total_ingresos :=
            (valor_mes1 + valor_mes2 + valor_mes3 + valor_mes4 + valor_mes5 + valor_mes6 + valor_mes7 + valor_mes8 +
             valor_mes9 + valor_mes10 + valor_mes11 + valor_mes12);
    ...
    if tipo = 'A' then
update rol_pago rl
set "ProvDecimoCuarto"      = valor_mes12,
    "acumuladoDecimoTercero"= total_pagar
where rl.id = id_rol_pago_dic;
else
update rol_pago rl
set "pagoDecimoCuartoMes" = total_pagar,
    "ProvDecimoCuarto"    = valor_mes12
where rl.id = id_rol_pago_dic;
end if;
...
create or replace function calcular_utilidades(fecha_inicio date, fecha_fin date, id_periodo integer,
                                               id_pago_uno integer) returns text
    language plpgsql
as
$$
DECLARE
...
select sum(cc."actualSaldo")
into valor_4
from cuenta_contable cc
where cast(cc.identificador as text) like '4%'
  and cc."idPeriodoContable" = id_periodo_contable;
select sum(cc."actualSaldo")
into valor_5
from cuenta_contable cc
where cast(cc.identificador as text) like '5%'
  and cc."idPeriodoContable" = id_periodo_contable;
...
for registro_trabajador in (select * from trabajador t where t."sisHabilitado" = 'A')
        loop
            ...
            if fecha_ingreso is not null then
                ...
                trabajador_aux := jsonb_build_object('id', registro_trabajador.id, 'fechaIngreso', fecha_ingreso,
                                                     'totalDiasTrabajados', total_dias_trabajados_anio);
...
FOR item in SELECT * FROM jsonb_array_elements((trabajadores_calcular_utilidad ->> 'trabajadores')::jsonb)
                              loop
...
insert into pagos2 ("sisActualizado", "sisCreado", "sisHabilitado", "anioPago", anticipos,
                    "diasLaboradosAlAnio", "fechaActual",
                    "fechaFin", "fechaInicio", multas, "otrosDescuentos", "otrosIngresos",
                    "prestamosEmpresa", "totalEgresos",
                    "totalIngresos", "valorAPagar", "valorMes1", "valorMes10", "valorMes11", "valorMes12",
                    "valorMes2", "valorMes3",
                    "valorMes4", "valorMes5", "valorMes6", "valorMes7", "valorMes8", "valorMes9",
                    "valorNominal", "valorReal",
                    "idPagosUno", "idPeriodoLaboral", "idTrabajador")

values (current_timestamp, current_timestamp, 'A', anio_periodo, 0, total_dias_trabajados_anio,
        current_date, fecha_fin,
        fecha_inicio, 0, 0, otros_ingresos, valor_prestamos_empresa, valor_prestamos_empresa,
        total_ingresos,
...
reporte_item := jsonb_build_object('idTrabajador', id_trabajador, 'valorReal', valor_real, 'utilidadcargas',
                                               otros_ingresos, 'diasLaborados', total_dias_trabajados_anio, 'utilidad',
                                               utilidad);
...
create or replace function generar_balance_comprobacion(anio_periodo integer, cuenta_inicio integer, cuenta_fin integer,
                                                        id_mes_inicio integer, id_mes_fin integer) returns text
    language plpgsql
as
$$
DECLARE
...
mes_inicio := (select public.obtener_mes(id_mes_inicio));
    ...
    consulta := 'select to_jsonb(array_agg(j)) from (select cc.nombre nombre, cc.identificador codigo,'
                    ...
                    || ' from cuenta_contable cc  where cast(cc.identificador as text)  ~ (''^[' || cuenta_inicio ||
                '-' || cuenta_fin || ']'')) j';
...
BEGIN
UPDATE contrato
SET "idContratos" = ARRAY_APPEND("idContratos", id_contrato_ant)
WHERE id = id_contrato_act;
...
create or replace function generar_balance_general(anio_periodo integer, identificador_cuenta character varying, tipo_movimiento character varying, identificador_nivel integer) returns text
    language plpgsql
as
$$
DECLARE
consulta           text;
    ...
    mes_fin            varchar(255);
    mes_anterior       varchar(255);
    balance            jsonb := jsonb_build_object('balance', jsonb_build_array());
    --balance            jsonb := jsonb_build_object();
   -- balance            jsonb := jsonb_build_object();
    result             text  := '';
    total_cuenta_mayor numeric;
    json_padre         jsonb;
    json_hijo          jsonb;
    registro_padre     record;
    counter            numeric;
    ruta               text[];
    ruta2               text;
    el jsonb;
...
select pc.id into id_periodo from periodo_contable pc where pc.anio = anio_periodo;

/*    select cc.identificador, cc.nombre, cc.nivel, coalesce(sum(cc."actualSaldo"), 0) saldo
    from cuenta_contable cc
    where cast(cc.identificador as text) ~ ('^[1-3]')
      and cc."idPeriodoContable" = id_periodo
      and cc.movimiento = tipo_movimiento
      and cc.nivel = identificador_nivel
    group by cc.identificador, cc.nombre, cc.identificador, cc.nivel
    order by cc.identificador asc;*/


/*    for n in 1..5
        loop
            for registro_padre in select cc.identificador,
                                         cc.nombre,
                                         cc.nivel,
                                         coalesce(sum(cc."actualSaldo"), 0) saldo
                                  from cuenta_contable cc
                                  where cast(cc.identificador as text) like '' || identificador_cuenta || '%'
                                    and cc."idPeriodoContable" = 2
                                    and cc.movimiento = 'M'
                                    and cc.nivel = n
                                  group by cc.identificador, cc.nombre, cc.identificador, cc.nivel
                                  order by cc.identificador asc
                loop
                    ...
                    json_padre := json_build_object('identificador', registro_padre.identificador, 'nombre',
                                                    registro_padre.nombre, 'saldo', 0, 'nivel', registro_padre.nivel,
                        'cuentasMayor', jsonb_build_array(),
                                                    'cuentasDeudoras', jsonb_build_array());

                    json_hijo := (select to_jsonb(array_agg(j))
                                  from (select cc.identificador,
                                               cc.nombre,
                                               cc.nivel,
                                               coalesce(sum(cc."actualSaldo"), 0) saldo
                                        from cuenta_contable cc
                                        where cast(cc.identificador as text) like
                                              '' || registro_padre.identificador || '%'
                                          and cc."idPeriodoContable" = 2
                                          and cc.movimiento = 'A'
                                          and cc.nivel = n + 1
                                        group by cc.identificador, cc.nombre, cc.identificador, cc.nivel
                                        order by cc.identificador asc) j);


                    IF json_hijo IS NOT NULL THEN
                        select sum(j.saldo::numeric)
                        into total_cuenta_mayor
                        from (SELECT jsonb_path_query(json_hijo, '$[*].saldo') saldo) j;
                        json_padre := jsonb_set(json_padre, '{cuentasDeudoras}', json_hijo, true);
                        json_padre := jsonb_set(json_padre, '{saldo}', to_jsonb(total_cuenta_mayor), true);
                    end if;


                    --RAISE NOTICE 'identificador %, cuenta %, nivel %, saldo % ', registro_padre.identificador, registro_padre.nombre, registro_padre.nivel, registro_padre.saldo;
                    RAISE NOTICE 'json % ', json_padre;

                    counter := jsonb_array_length((balance ->> 'balance')::jsonb);
                    ruta := '{balance,' || jsonb_array_length((balance ->> 'balance')::jsonb) + 1 || '}';
--                    RAISE NOTICE 'path a buscar %', ruta;
                    balance := jsonb_insert(balance, ruta, json_padre, true);

                    /*if n = 1 then
                        balance := jsonb_insert(balance, '{balance}', json_padre, true);
                    else
                         balance := jsonb_set(balance, '{cuentasMayor,1}', json_padre, true);
                    end if;*/
...
select jsonb_pretty((balance->>'balance')::jsonb) into result;


/*      FOR el IN SELECT * FROM json_array_elements((balance ->> 'balance')::json) LOOP

          counter := el->>'nivel'::numeric;
          balance := jsonb_set(json_padre, '{saldo}', to_jsonb(total_cuenta_mayor), true);

          select sum(j.saldo::numeric)
                        into total_cuenta_mayor
                        from (SELECT jsonb_path_query(json_hijo, '$[*].saldo') saldo) j;

          RAISE NOTICE 'output from space %', el;
      END LOOP;*/
...
/*    for x in reverse 6..3 loop
        ruta2 := '$.balance[*] ? (@.nivel == ' || x - 1 || ')';
        ruta := '{balance,'||x - 2||',saldo}';
        RAISE NOTICE 'path a actualizar %', ruta;
        --RAISE NOTICE 'JSON %', jsonb_path_query(balance,'$.balance[*] ? (@.nivel == $n)','{ "n" : '|| x ||' }');

        --select z.json into json_hijo from (select jsonb_path_query(balance::json,'$.balance[*] ? (@.nivel == '|| x ||')') json) z;
        --RAISE NOTICE 'suma % ', json_hijo;

        select sum(j.saldo::numeric)
                        into total_cuenta_mayor
                        from (SELECT jsonb_path_query(balance, /*'$.balance[*] ? (@.nivel == 1)'*/ ruta2::jsonpath)::jsonb ->> 'saldo' saldo) j;

        RAISE NOTICE 'total actualizar en padre %', total_cuenta_mayor;

        balance := jsonb_set(balance, ruta, to_jsonb(total_cuenta_mayor), true);


    end loop;*/

RAISE NOTICE 'balance final % ', result;


    /*for n in reverse 4..1
        loop
            ...
            for registro_padre in select cc.identificador,
                ...
                json_padre := json_build_object('identificador', registro_padre.identificador, 'nombre',
                                                    registro_padre.nombre, 'saldo', 0, 'nivel', registro_padre.nivel,
                                                    'cuentasAdeudoras', jsonb_build_array());
                 ...
                 IF json_hijo IS NOT NULL THEN
                        ...
                        into total_cuenta_mayor
                        from (SELECT jsonb_path_query(json_hijo, '$[*].saldo') saldo) j;
                        json_padre := jsonb_set(json_padre, '{cuentasAdeudoras}', json_hijo, true);
                        json_padre := jsonb_set(json_padre, '{saldo}', to_jsonb(total_cuenta_mayor), true);
                    end if;
...
*/
    /*for registro in select cc.identificador, cc.nombre, cc.nivel, coalesce(sum(cc."actualSaldo"), 0) saldo
                    from cuenta_contable cc
                    where cast(cc.identificador as text) ~ (identificador_cuenta)
                      and cc."idPeriodoContable" = 2
                      and cc.movimiento = tipo_movimiento
                      and cc.nivel = identificador_nivel
                    group by cc.identificador, cc.nombre, cc.identificador, cc.nivel
                    order by cc.identificador asc
        loop
        RAISE NOTICE 'identificador %, cuenta %, nivel % ', registro.identificador, registro.nombre, registro.nivel;

        perform public.generar_balance_general(2023, '^['||SUBSTRING(registro.identificador::text, 1, 2)||']', 'A', 2);
...
$$;
 ...
 --- CUOTA REFINANCIAMIENTO ---

create or replace function cuotas_refinanciamiento(cuota_par cuota, plan_par plan, valor_cuota_par numeric, abono_capital_par numeric, tasa_administrativa_par numeric, impuesto_par numeric, resto_inicio_par numeric, numero_cuotas_inicio_par numeric, resto_final_par numeric, pms_par integer, id_historico_par integer) returns void
    language plpgsql
as
$$
Declare
...
cont:=cont+1;
    ...
    $$;
---- ASIENTOS CAMBIOS 1 -----
create or replace function asiento_cambios1(w_idperiodo integer, w_anio integer, w_fechainicio date, w_nombrescliente character varying, westado_contrato character varying, w_fechadeposito date, nombrecolumna character varying, w_mesperiodo character varying, w_ivaporcentaje numeric, w_valor numeric) returns void
    language plpgsql
as
$$
declare
...
w_idCuentaContableCxC integer;
w_idCuentaContableIva integer;
w_idCuentaContableIngreso integer;
w_idCuentaContableBco  integer;
w_idCuentaContableNoAdjudi integer;
w_nombreCuentaContable character varying(255);
...
VALUES (
           ...
           'Ajuste '||westado_contrato, w_fechaDeposito, w_mesPeriodo, 'E',w_TipoTransaccion,
           0,0,0, w_idCuentaContableCxC)
    RETURNING id INTO id_AsientoContableCabecera;
end;
...
begin
select id
into w_idCuentaContableIngreso ----   como se afecta el actualsaldo,  a que cuenta esta relacionado
from cuenta_contable
where 	"idPeriodoContable"=w_idperiodo and
    "codigoCuentaContable" in (
        select trim("codigoCuentaContable")
        from item_cobro_pago
        where "nombre_cuenta"='INGRESO');
end;
begin
INSERT INTO public.transaccion_asiento_contable(
    "sisActualizado", "sisCreado", "sisHabilitado", detalle,
    "valorCredito", "valorDebito", "idAsientoContableCabecera", "idCuentaContable")
VALUES (now(),now(),'A', 'INGRESO',
        w_diferencia, 0, id_AsientoContableCabecera, w_idCuentaContableIngreso)
    RETURNING id INTO id_transaccionAsientoContable;
end;
begin
	perform public.actualiza_saldo_ctacble(	w_idCuentaContableIngreso, nombreColumna||'Credito"', w_diferencia);
end;
else
...
from cuenta_contable
where "idPeriodoContable"=w_idperiodo and
    "codigoCuentaContable"  in (select trim("codigoCuentaContable")
                                from item_cobro_pago
                                where nombre_cuenta='NO ADJUDICADO AUTOS');
end;
begin
INSERT INTO public.transaccion_asiento_contable(
    ...
    "valorCredito", "valorDebito", "idAsientoContableCabecera", "idCuentaContable")
VALUES (now(),now(),'A', 'NO ADJUDICADO AUTOS',
        0, w_valor,  id_AsientoContableCabecera, w_idCuentaContableNoAdjudi)   --- 2 es valor quemado para esta cuenta
    RETURNING id INTO id_transaccionAsientoContable;
end;

		raise notice  'value of w_idCuentaContableNoAdjudi : %', w_idCuentaContableNoAdjudi;
begin
		perform public.actualiza_saldo_ctacble(	w_idCuentaContableNoAdjudi, nombreColumna||'Debito"', w_valor);
end;
...
--- CAMBIOS 2 ---
create or replace function asiento_cambios2(w_idperiodo integer, w_anio integer, w_fechainicio date, w_nombrescliente character varying, westado_contrato character varying, w_fechadeposito date, nombrecolumna character varying, w_mesperiodo character varying, w_ivaporcentaje numeric, w_valor numeric) returns void
    language plpgsql
as
$$
declare
...
INSERT INTO public.transaccion_asiento_contable(
    ...
    "valorCredito", "valorDebito", "idAsientoContableCabecera", "idCuentaContable")
VALUES (now(),now(),'A', 'IVA POR PAGAR',
        0, w_valor_iva,  id_AsientoContableCabecera, w_idCuentaContableIva)   --- 3 es valor quemado para esta cuenta
    RETURNING id INTO id_transaccionAsientoContable;
end;

begin
	perform public.actualiza_saldo_ctacble(	w_idCuentaContableIva, nombreColumna||'Debito"', w_valor_iva);
end;
...
INSERT INTO public.transaccion_asiento_contable(
    ...
    "valorCredito", "valorDebito", "idAsientoContableCabecera", "idCuentaContable")
VALUES (now(),now(),'A', 'INGRESO',
        0, w_diferencia,  id_AsientoContableCabecera, w_idCuentaContableIngreso)
    RETURNING id INTO id_transaccionAsientoContable;
end;
begin
	perform public.actualiza_saldo_ctacble(	w_idCuentaContableIngreso, nombreColumna||'Debito"', w_diferencia);
end;
else
...
VALUES (now(),now(),'A', 'NO ADJUDICADO AUTOS',
    ...
    RETURNING id INTO id_transaccionAsientoContable;
		...
		raise notice  'value of w_idCuentaContableNoAdjudi : %', w_idCuentaContableNoAdjudi;
begin
		perform public.actualiza_saldo_ctacble(	w_idCuentaContableNoAdjudi, nombreColumna||'Credito"', w_valor);
end;
...
--- UNIFICACION CONTRATOS BACK ---
create or replace function unificacion_contratoback(jsonpar character varying) returns character varying
    language plpgsql
as
$$
DECLARE
...
*/
	...
	abono_capital := (nuevo_plan.precio/pms);
	 raise notice 'value of abono_capital : %',abono_capital;
    iva := (cg."ivaPorcentaje"/100);
	raise notice 'value of iva  : %',iva ;
     tasa_administrativa := round(((nueva_cuota - abono_capital)/(iva + 1)),3);
	 raise notice 'value of nueva_cuota : %',nueva_cuota ;

	 raise notice 'value of tasa_administrativa : %',tasa_administrativa;
    impuesto := tasa_administrativa * iva;
...
*/
    ...
    total_pagado_segundo_plan := hpc2."totalMontoCobrado" - cuota2."valorCuota";
    total_pagado_planes := total_pagado_primer_plan + total_pagado_segundo_plan;
	raise notice  'value of total_pagado_primer_plan: %', total_pagado_primer_plan;
	raise notice  'value of total_pagado_segundo_plan: %', total_pagado_segundo_plan;

	raise notice  'value of total_pagado_planes: %', total_pagado_planes;
    ...
    tasa_administrativa_restante := 0;
    abonos_capital_actual := numero_cuotas_pasar * (abono_capital);
	raise notice  'value of numero_cuotas_pasar: %', numero_cuotas_pasar;
...
*/
    -- comprobaciones del calculo correcto
    capital1 := (cuota1."abonoCapital") * (hpc1."totalCuotasCobradas" - 1);
	raise notice  'value of capital1: %', capital1;
	capital2 := (cuota2."abonoCapital") * (hpc2."totalCuotasCobradas" - 1);
	raise notice  'value of cuota2."abonoCapital": %', cuota2."abonoCapital";
	raise notice  'value of hpc2."totalCuotasCobradas": %', hpc2."totalCuotasCobradas";
	raise notice  'value of capital2: %', capital2;
	capital3 := (abonos_capital_actual);
	raise notice  'value of capital3: %', capital3;

    saldo_pagado_tasa := round((select (SELECT abs((capital1 + capital2) - capital3))),2);
    ...
    diferencia := round((tasa_administrativa1 + tasa_administrativa2 - tasa_administrativa3),2);
    valor_absoluto := (SELECT (SELECT abs(diferencia)));

	 raise notice  'value of saldo_pagado_tasa : %', saldo_pagado_tasa;
	 raise notice  'value of valor_absoluto : %', valor_absoluto;
    if (saldo_pagado_tasa <> valor_absoluto) then
	    RAISE INFO 'value of saldo_pagado_tasa : %', saldo_pagado_tasa;
		--RAISE INFO 'valor_absoluto : %', valor_absoluto;
RETURN '{"correcto":false, "message":"Error en los calculo de comprobacion de la diferencia de la tase administrativa y el capital"}',saldo_pagado_tasa::text ;
end if;
